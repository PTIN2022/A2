...
def post_login(email, password):
    ...
    if t:
        if t.password == password:
            trabajador = TrabajadorSchema().dump(t)
            trabajador["token"] = jwt.encode({"email": t.email, "rol": t.cargo, "exp": expire_date(app.config['EXPIRE_TOKEN_TIME'])}, app.config['SECRET_KEY'], algorithm="HS256")
            return trabajador
